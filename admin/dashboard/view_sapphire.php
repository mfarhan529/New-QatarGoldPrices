...
function getSapphireByCurrency($conn, $currency_symbol) {
    $sql = "
        SELECT s.id, s.type, s.Price_Range, s.created_at,
               w.unit AS weight_unit, c.Symbol
        FROM sapphire_prices s
        INNER JOIN (
        ...
        ON s.type = latest.type 
           AND s.weight_id = latest.weight_id 
           AND s.currency_id = latest.currency_id 
           AND s.created_at = latest.latest_date
        LEFT JOIN currencies c ON s.currency_id = c.id
        LEFT JOIN weight w ON s.weight_id = w.id
        WHERE c.Symbol = ?
...
<head>
  <meta charset="UTF-8">
  <title>View Sapphire Rates</title>
  <style>
...
<div class="table-wrapper">
  ...
  <?php foreach (["QAR","INR","USD"] as $currency_symbol): ?>
    <div class="table-container">
      <h2> Sapphire Prices (<?= $currency_symbol ?>)</h2>
      <table>
        <thead>
          <tr>
            <th>Date</th>
        ...
        <tbody>
          <?php 
          $result = getSapphireByCurrency($conn, $currency_symbol);
