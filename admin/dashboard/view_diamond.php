...
function getDiamondByCurrency($conn, $currency_symbol) {
    $sql = "
        SELECT d.id, d.type, d.Price_Range, d.created_at,
               w.unit AS weight_unit, c.Symbol
        FROM diamond_prices d
        INNER JOIN (
            SELECT type, weight_id, currency_id, MAX(created_at) AS latest_date
            FROM diamond_prices
            GROUP BY type, weight_id, currency_id
        ) latest 
        ON d.type = latest.type 
           AND d.weight_id = latest.weight_id 
           AND d.currency_id = latest.currency_id 
           AND d.created_at = latest.latest_date
        LEFT JOIN currencies c ON d.currency_id = c.id
        LEFT JOIN weight w ON d.weight_id = w.id
        WHERE c.Symbol = ?
...
<head>
  <meta charset="UTF-8">
  <title>View Diamond Rates</title>
  <style>
...
<div class="table-wrapper">
  ...
  <?php foreach (["QAR","INR","USD"] as $currency_symbol): ?>
    <div class="table-container">
      <h2>💎 Diamond Prices (<?= $currency_display[$currency_symbol] ?>)</h2>
      <table>
        <thead>
          <tr>
            <th>Date</th>
        ...
        <tbody>
          <?php 
          $result = getDiamondByCurrency($conn, $currency_symbol);
