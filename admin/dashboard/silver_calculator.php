...
// ✅ Fetch allowed weight units (1 Gram, 1 Tola, 1 Ounce, 1 Kg)
$weights = [];
$w_query = $conn->query("SELECT id, unit FROM weight WHERE unit IN ('1 Gram','1 Tola','1 Ounce','1 Kg')");
    ...
    if ($user_weight <= 0) {
        ...
        foreach ($currencies as $symbol => $currency_id) {
            $stmt = $conn->prepare("
                SELECT Prices 
                FROM silver_prices 
                WHERE weight_id = ? AND currency_id = ? 
                ORDER BY created_at DESC 
                LIMIT 1
            ");
            $stmt->bind_param("ii", $weight_id, $currency_id);
            $stmt->execute();
        ...
        if (!empty($results)) {
            ...
            foreach ($results as $symbol => $value) {
                // Remove leading "1 " from unit
                $unit_clean = preg_replace('/^1\s*/', '', $weights[$weight_id]);

                $result_msg .= "<li>The price of {$user_weight} {$unit_clean} of Silver in {$symbol} is <strong>{$value}</strong></li>";
            }
            $result_msg .= "</ul></div>";
        } else {
            $result_msg = "<p style='color:red;font-weight:bold;'>❌ Latest silver price not available in database.</p>";
        }
    }
}
?>
...
<head>
  <meta charset="UTF-8">
  <title>Silver Rate Calculator</title>
  <style>
    body {
