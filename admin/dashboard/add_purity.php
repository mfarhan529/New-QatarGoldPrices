...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $name = trim($_POST['name']); // ✅ Match with form name
    
    if (empty($name)) {
        $error = "❌ Name cannot be empty.";
    } else {
        $sql = "INSERT INTO purities (`name`) VALUES (?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("s", $name);

        if ($stmt->execute()) {
            header("Location: view_purity.php?success=1");
            exit();
        } else {
  ...
  <div class="form-container">
    ...
    <form method="POST" action="">
     ...
     <div class="form-group">
        <label for="Purity">Enter Purity</label>
        <input 
          type="text" 
          name="name" 
          id="name" 
      ...
      <button type="submit">💰 Save Purity</button>
    </form>
