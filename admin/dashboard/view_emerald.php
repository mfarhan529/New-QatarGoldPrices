...
function getEmeraldByCurrency($conn, $currency_symbol) {
    $sql = "
        SELECT e.id, e.type, e.Price_Range, e.created_at,
               w.unit AS weight_unit, c.Symbol
        FROM emerald_prices e
        INNER JOIN (
            SELECT type, weight_id, currency_id, MAX(created_at) AS latest_date
            FROM emerald_prices
            GROUP BY type, weight_id, currency_id
        ) latest 
        ON e.type = latest.type 
           AND e.weight_id = latest.weight_id 
           AND e.currency_id = latest.currency_id 
           AND e.created_at = latest.latest_date
        LEFT JOIN currencies c ON e.currency_id = c.id
        LEFT JOIN weight w ON e.weight_id = w.id
        WHERE c.Symbol = ?
...
<head>
  <meta charset="UTF-8">
  <title>View Emerald Rates</title>
  <style>
...
<div class="table-wrapper">
  ...
  <?php foreach (["QAR","INR","USD"] as $currency_symbol): ?>
    <div class="table-container">
      <h2> Emerald Prices (<?= $currency_symbol ?>)</h2>
      <table>
        <thead>
          <tr>
            <th>Date</th>
        ...
        <tbody>
          <?php 
          $result = getEmeraldByCurrency($conn, $currency_symbol);
